...
CREATE TABLE talat_driver (
  ...
  cyear      VARCHAR(10)  NOT NULL COMMENT '차량연식', -- 차량연식
  ...
  apvl       INTEGER      NOT NULL DEFAULT 0 COMMENT '승인여부', -- 승인여부
...
CREATE TABLE talat_join (
  ...
  star_avg    INTEGER      NOT NULL DEFAULT 5.0 COMMENT '별점통계', -- 별점통계
...
CREATE TABLE talat_journey_rider (
  ...
  jno   INTEGER  NOT NULL COMMENT '여정번호', -- 여정번호
  mno   INTEGER  NOT NULL COMMENT '회원번호', -- 회원번호
  mstat INTEGER  NOT NULL DEFAULT 0 COMMENT '매칭상태', -- 매칭상태
...
ALTER TABLE talat_review
  ADD CONSTRAINT CK_talat_review -- 리뷰메세지 체크 제약
    CHECK (rev_no = 1 or rev_no = 2 or rev_no = 3 or rev_no = 4 or rev_no = 5 or rev_no = 6 or rev_no = 0);
...
CREATE TABLE talat_review_driver (
  ...
  rev_no   INTEGER      NOT NULL COMMENT '리뷰메세지 번호', -- 리뷰메세지 번호
  rev      VARCHAR(100) NOT NULL COMMENT '내용', -- 내용
  rstar    INTEGER      NOT NULL COMMENT '라이더별점', -- 라이더별점
  drevdate DATETIME     NOT NULL DEFAULT now() COMMENT '리뷰등록일', -- 리뷰등록일
  drevstat INTEGER      NOT NULL DEFAULT 0 COMMENT '리뷰여부' -- 리뷰여부
)
COMMENT '드라이버평가';
...
-- 드라이버평가
ALTER TABLE talat_review_driver
  ADD CONSTRAINT CK_talat_review_driver -- 드라이버평가 체크 제약
    CHECK (rstar = 1 or rstar = 2 or rstar = 3 or rstar = 4 or rstar = 5);

-- 드라이버평가
ALTER TABLE talat_review_driver
  ADD CONSTRAINT CK_talat_review_driver2 -- 드라이버평가 체크 제약2
    CHECK (drevstat = 1 or drevstat = 0);
...
CREATE TABLE talat_review_rider (
  ...
  rev_no   INTEGER      NOT NULL COMMENT '리뷰메세지 번호', -- 리뷰메세지 번호
  rev      VARCHAR(100) NOT NULL COMMENT '내용', -- 내용
  dstar    INTEGER      NOT NULL COMMENT '드라이버별점', -- 드라이버별점
  rrevdate DATETIME     NOT NULL DEFAULT now() COMMENT '리뷰등록일', -- 리뷰등록일
  rrevstat INTEGER      NOT NULL DEFAULT 0 COMMENT '리뷰여부' -- 리뷰여부
)
COMMENT '라이더평가';
...
-- 라이더평가
ALTER TABLE talat_review_rider
  ADD CONSTRAINT CK_talat_review_rider -- 라이더평가 체크 제약
    CHECK (dstar = 1 or dstar = 2 or dstar = 3 or dstar = 4 or dstar = 5);

-- 라이더평가
ALTER TABLE talat_review_rider
  ADD CONSTRAINT CK_talat_review_rider2 -- 라이더평가 체크 제약2
    CHECK (rrevstat = 1 or rrevstat = 0);
